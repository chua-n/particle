...
class VAE2d(nn.Module):
    def __init__(self, n_latent=32, lamb=1, lr=1e-3, batch_norm=True, mode='train'):
        ...
        self.fc_features1 = 32 * 100 * 100
        self.fc_features2 = 80
        self.fc1 = nn.Linear(self.fc_features1, self.fc_features2, bias=True)

        self.fc_z1 = nn.Linear(self.fc_features2, self.n_latent, bias=True)
        self.fc_z2 = nn.Linear(self.fc_features2, self.n_latent, bias=True)

        self.fc2 = nn.Linear(self.n_latent, self.fc_features2, bias=True)
        self.fc3 = nn.Linear(self.fc_features2, self.fc_features1, bias=True)
        self.conv_transpose1 = self.conv_transpose2d(32, 16, 5, 2,
    ...
    def encoder(self, source):
        ...
        mu = self.fc_z1(source)
        log_sigma = self.fc_z2(source)
        return mu, log_sigma
...
def test_shape(model, source):
    ...
    re = vae.fc2(z)
    re = vae.fc3(re)
    re = re.view(re.shape[0], -1, 100, 100)
