...
class Generator(nn.Module):
    def __init__(self, n_noise=62, n_disc=6, n_cont=5):
        super().__init__()
        self.n_latent = n_noise + n_disc + n_cont

        # n_latent * 1 * 1
        self.conv_transpose1 = nn.Sequential(
            nn.ConvTranspose2d(self.n_latent, ngf * 8, 4, 1,
        ...
        self.conv_transpose4 = nn.Sequential(
            nn.ConvTranspose2d(ngf * 2, ngf, 4, 2, padding=1, bias=False),
            nn.BatchNorm2d(ngf),
            nn.ReLU())

        # ngf * 32 * 32
        self.conv_transpose5 = nn.Sequential(
            nn.ConvTranspose2d(ngf, 1, 4, 2, padding=1, bias=False),
            nn.Sigmoid())  # nn.tanh???
...
class Share(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, ndf, 4, 2, padding=1, bias=False),
            # nn.BatchNorm2d(ndf),  # why have no BN layer
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv5 = nn.Sequential(
            nn.Conv2d(ndf * 8, 1, 4, 1, padding=0, bias=False),
            nn.Sigmoid())
        # finally: 1 * 1 * 1
...
class Q(nn.Module):
    def __init__(self, n_disc=6, n_cont=5):
        super().__init__()
        self.n_disc = n_disc
        self.n_cont = n_cont
        # 此层暴力迁移过来的
        self.conv5 = nn.Sequential(
            nn.Conv2d(ndf * 8, 128, 4, 1, padding=0, bias=False),
            nn.BatchNorm2d(128),
            nn.LeakyReLU(0.2))
...
for epoch in range(N_EPOCH):
    for i, (x,) in enumerate(train_set):
        ...
        loss_G_disc = criterion_disc(q_disc, torch.max(disc, 1).indices)
        cont = z[:, -N_CONT:]
    ...
    with torch.no_grad():
        for i, fixed_z in enumerate([fixed_z1, fixed_z2, fixed_z3, fixed_z4, fixed_z5]):
            generations = net_G(fixed_z)
            save_image(generations, '{}-fixed_z{}.png'.format(epoch + 1, i + 1), nrow=6)
