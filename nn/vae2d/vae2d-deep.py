...
class VAE2d(nn.Module):
    def __init__(self, n_latent=32, lamb=1, lr=1e-3, batch_norm=True, mode='train'):
        ...
        self.conv1 = self.conv2d(1, 8, 4, 2,    # 200*200
                                 padding=0, bias=False)
        self.conv2 = self.conv2d(8, 16, 4, 2,
                                 padding=1, bias=False)
        self.conv3 = self.conv2d(16, 32, 4, 2,
                                 padding=1, bias=False)
        self.conv4 = self.conv2d(32, 64, 4, 2,
                                 padding=1, bias=False)
        self.conv5 = self.conv2d(64, 128, 4, 2,
                                 padding=1, bias=False)
        self.conv6 = self.conv2d(128, 128, 4, 2,
                                 padding=1, bias=False)
        self.conv7 = self.conv2d(128, 128, 4, 2,
                                 padding=1, bias=False)
        self.conv8 = self.conv2d(128, 256, 3, 1,
                                 padding=0, bias=False)

        self.fc_features = 256
        ...
        self.conv_transpose1 = self.conv_transpose2d(256, 128, 3, 1,
                                                     padding=0, output_padding=0, bias=False)
        self.conv_transpose2 = self.conv_transpose2d(128, 128, 4, 2,
                                                     padding=1, output_padding=0, bias=False)
        self.conv_transpose3 = self.conv_transpose2d(128, 128, 4, 2,
                                                     padding=1, output_padding=0, bias=False)
        self.conv_transpose4 = self.conv_transpose2d(128, 64, 4, 2,
                                                     padding=1, output_padding=1, bias=False)
        self.conv_transpose5 = self.conv_transpose2d(64, 32, 4, 2,
                                                     padding=1, output_padding=0, bias=False)
        self.conv_transpose6 = self.conv_transpose2d(32, 16, 4, 2,
                                                     padding=1, output_padding=0, bias=False)
        self.conv_transpose7 = self.conv_transpose2d(16, 8, 4, 2,
                                                     padding=1, output_padding=0, bias=False)
        self.conv_transpose8 = nn.Sequential(
            nn.ConvTranspose2d(8, 1, 4, 2,
                               padding=0, output_padding=0, bias=True, padding_mode='zeros'),
...
def test_shape(model, source):
    ...
    re = vae.fc3(z)
    re = re.view(*re.shape, 1, 1)
    re = vae.conv_transpose1(re)
    re = vae.conv_transpose2(re)
    re = vae.conv_transpose3(re)
    re = vae.conv_transpose4(re)
    re = vae.conv_transpose5(re)
    re = vae.conv_transpose6(re)
    re = vae.conv_transpose7(re)
    re = vae.conv_transpose8(re)
    print(re)
