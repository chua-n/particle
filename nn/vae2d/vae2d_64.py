...
class VAE2d(nn.Module):
    def __init__(self, n_latent=32, lamb=1, lr=1e-3, batch_norm=True, mode='train'):
        ...
        self.conv2 = self.conv2d(16, 32, 5, 2,
                                 padding=2, bias=False)
        self.conv3 = self.conv2d(32, 64, 5, 2,
                                 padding=2, bias=False)
        self.conv4 = self.conv2d(64, 64, 5, 2,
                                 padding=2, bias=False)
        self.conv5 = self.conv2d(64, 128, 4, 1,
        ...
        self.conv_transpose1 = self.conv_transpose2d(128, 64, 4, 1,
                                                     padding=0, output_padding=0, bias=False)
        self.conv_transpose2 = self.conv_transpose2d(64, 64, 5, 2,
                                                     padding=2, output_padding=1, bias=False)
        self.conv_transpose3 = self.conv_transpose2d(64, 32, 5, 2,
                                                     padding=2, output_padding=1, bias=False)
        self.conv_transpose4 = self.conv_transpose2d(32, 16, 5, 2,
                                                     padding=2, output_padding=1, bias=False)
...
def test_shape(model, source):
    ...
    mu, log_sigma = vae2d.encoder(source)
    print('after encoder:', mu.shape, log_sigma.shape)
    z = vae2d.reparameterize(mu, log_sigma)
    print('size of coding:', z.shape)

    re = nn.Linear(vae2d.n_latent, 128, bias=True)(z)
    re = re.view(*re.shape, 1, 1)
    print('before transpose:', re.shape)
    re = vae2d.conv_transpose1(re)
    print('after conv_transpose1:', re.shape)
    re = vae2d.conv_transpose2(re)
    print('after conv_transpose2:', re.shape)
    re = vae2d.conv_transpose3(re)
    print('after conv_transpose3:', re.shape)
    re = vae2d.conv_transpose4(re)
    print('after conv_transpose4:', re.shape)
    re = vae2d.conv_transpose5(re)
    print('after conv_transpose5:', re.shape)
...
def main():
    ...
    for epoch in range(N_EPOCH):
        for i, (x,) in enumerate(train_set):
            ...
            loss.backward()
            optimizer.step()
            if (i + 1) % 50 == 0 or (i + 1) == len(train_set):
