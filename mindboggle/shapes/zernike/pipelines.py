...
class SerialPipeline(Pipeline):
    ...
    def Znlmp(self,N,x,y,z):
        ...
        for n, l, m, nu, in nest(lambda: range(N + 1),
            ...
            k = int((n - l) / 2) 
            temp = 0
            for j in range(int((l - m) / 2) + 1):
                temp += self.Yljm(l,j,m)*(x**2+y**2+z**2)**(nu+j)*z**(l-m-2*j)*(x+IMAG_CONST*y)**m
            Z[n,l,m] += self.Qklnu(k,l,nu)*temp
...
if __name__ == "__main__":
    for n, l, m in nest(lambda: range(5 + 1),
                            lambda _n: range(_n + 1),
                            lambda _n, _l: range(_l + 1),
                            ):
        print(n,l,m)
